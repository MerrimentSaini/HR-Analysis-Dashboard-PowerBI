# 📊 HR Analysis Dashboard | Power BI

<===== 📌 Project Overview =====>

This HR Analysis Dashboard is an interactive Power BI project designed to analyze workforce performance, employee demographics, salaries, and attrition trends. The dashboard provides actionable insights that help HR teams understand employee behavior and identify the major factors contributing to attrition.

---

# 📷 Dashboard Preview

## Dashboard - Page 1

![HR Analysis Dashboard - Page 1](Images/HR_Analysis_Dashboard_Page_1.png)

## Dashboard - Page 2

![HR Analysis Dashboard - Page 2](Images/HR_Analysis_Dashboard_Page_2.png)

---

# 🎯 Project Objectives

- Analyze employee attrition
- Identify high-risk departments
- Understand workforce demographics
- Compare active and exited employees
- Analyze salary distribution
- Study employee satisfaction
- Monitor overtime impact
- Explore business travel patterns

---

# 📈 Key KPIs

- 👨‍💼 Total Employees
- ✅ Active Employees
- ❌ Employees Left
- 📉 Attrition Rate
- 💰 Average Monthly Income
- 😊 Average Job Satisfaction

---

# 📊 Dashboard Insights

## Employee Attrition

- Department-wise Attrition
- Gender-wise Attrition
- Age Group Analysis
- Overtime Analysis

## Employee Demographics

- Marital Status
- Education Field
- Years at Company

## Salary Analysis

- Average Salary by Job Role

## Employee Behavior

- Business Travel vs Attrition
- Employee Experience by Years at Company

---

# 🛠 Tools Used

- Microsoft Power BI
- Power Query
- DAX
- Data Modeling
- Microsoft Excel / CSV Dataset

---

# 📂 Dataset

The dataset contains HR employee information including:

- Employee ID
- Department
- Gender
- Age
- Marital Status
- Education
- Job Role
- Monthly Income
- Overtime
- Attrition
- Years at Company
- Job Satisfaction

---

# 📌 Dashboard Features

- Interactive Filters
- Dynamic KPI Cards
- Donut Charts
- Bar Charts
- Line Charts
- Percentage Analysis
- Professional Dark Theme UI

---

# 📚 Skills Demonstrated

- Data Cleaning
- Data Transformation
- Data Modeling
- DAX Measures
- Dashboard Design
- Business Intelligence
- HR Analytics
- Storytelling with Data

---

# 🚀 Business Insights

- Research & Development shows the highest employee attrition.
- Employees working overtime are more likely to leave.
- Employees aged 26–35 contribute the highest attrition.
- Life Sciences has the highest attrition among education fields.
- Married employees represent the largest share of active employees.
- Salary varies significantly across different job roles.

---

# 📁 Repository Structure

```
HR-Analysis-Dashboard-PowerBI
│
├── Dashboard
│   └── HR_Analysis_Dashboard.pbix
│
├── Dataset
│   └── HR_Employee_Attrition.csv
│
├── Images
│   ├── Dashboard_Page1.png
│   └── Dashboard_Page2.png
│
└── README.md
```

---

<===== 👨‍💻 Author =====>

**Avi Saini**

BCA Student | Data Analyst Aspirant

GitHub: https://github.com/MerrimentSaini

LinkedIn: https://www.linkedin.com/in/avi-saini-15956b214

---

<===== ⭐ Support =====>

If you found this project useful, consider giving it a ⭐ on GitHub.
